...
实现状态（2026-03-26）：CN MVP 主链路已落地并稳定运行，当前测试集 `PYTHONPATH=. pytest -q` 共 52 项通过。

说明：本文档是 CN MVP 的目标设计说明；当前实现已达到“可运行 + 可恢复 + 可导出”的 MVP 目标，仍保留部分阶段内容质量上的 stub 语义。
...
4. 若阶段 metadata 快照缺失，可回退 `metadata_latest.json` 并尝试恢复 Stage 04 的人工决策
...
当前实现注记：上述条目在当前代码库已满足 MVP 级闭环；复杂法律校验与高质量全文撰写仍按后续迭代推进，具体以 README 的“当前实现范围”描述为准。
